...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white border border-outline-variant/40 rounded-lg shadow-editorial px-4 py-3 text-sm">
      <p className="font-semibold text-primary mb-1">{label}</p>
      {payload.map((p) => (
        <p key={p.dataKey} className="text-on-surface-variant text-xs">
  ...
  return (
    <div className="bg-white p-8 rounded-card shadow-editorial card-enter">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
        <div>
          <span className="text-label-sm text-outline uppercase tracking-wider">Resource Utilization</span>
          <h2 className="font-display text-headline-md text-primary">Time Breakdown</h2>
        </div>
        <div className="flex bg-[#F2EEE6] rounded-lg p-1 w-fit">
          {DIMENSIONS.map((d) => (
            <button
              key={d.key}
      ...
      {loading ? (
        <div className="h-64 flex items-center justify-center text-sm text-outline">Loading chart...</div>
      ) : data.length === 0 ? (
        <div className="h-64 flex items-center justify-center text-sm text-outline">No data for this filter.</div>
      ) : (
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} onClick={(e) => {
            ...
            <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#727973" }} angle={-20} textAnchor="end" height={70} axisLine={{ stroke: "#c1c8c2" }} />
            <YAxis tick={{ fontSize: 11, fill: "#727973" }} axisLine={false} tickLine={false} />
      ...
      {dimension === "task_category" && (
        <p className="text-xs text-outline mt-3">Click a bar to filter the employee list by that task.</p>
      )}
    </div>
  );
}